...
class OneOf(Validator):
    ...
    def validate(self, value):
        if value not in self.options:
            raise ValueError(f'Expected {value!r} to be one option of {self.options!r}')
...
class Number(Validator):
    ...
    def validate(self, value):
        if not isinstance(value, (int, float)):
            raise ValueError(f'Expected {value!r} to be an int or float')
...
class String(Validator):
    ...
    def validate(self, value):
        if not isinstance(value, str):
            raise ValueError(f'Expected {value!r} is to be an str')
